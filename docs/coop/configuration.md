...
### Mosquitto (`config/coop/mosquitto/`)
...
### ChirpStack (`config/coop/chirpstack/`)
...
Template (version-controlled): `config/coop/frigate/config.yml`, `config/coop/frigate/go2rtc_homekit.yml`.
...
Generated by `scripts/coop/coop-mosquitto-tls.sh`:
