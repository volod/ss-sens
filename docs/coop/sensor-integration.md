...
- Run `./scripts/test_usb_cameras.sh` to verify detection

### Step 1: Test USB Camera

```bash
./scripts/test_usb_cameras.sh
```
...
```bash
# Add RTSP camera
python scripts/add_camera.py --name front_door --rtsp rtsp://user:pass@ip:554/stream1 --restart
