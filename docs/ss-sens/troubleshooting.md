...
4. Ensure data directories exist (bind mounts):
   ```bash
   ./scripts/ss-sens/ss-sens-data-dirs.sh
   ```
...
5. If containers fail with "Operation not permitted" or "Unable to open" on data dirs:
   ...
   - For a fresh start: `./scripts/ss-sens/ss-sens-clean-data.sh`, then `./scripts/ss-sens/ss-sens-bootstrap.sh`

6. If you cannot remove `./data/*` (permission denied):
   - `./scripts/ss-sens/ss-sens-clean-data.sh` fixes ownership and removes data
...
3. Ensure TLS and pwfile were created by your user (not sudo):
   - `./scripts/ss-sens/ss-sens-mosquitto-tls.sh` - run as normal user
   - `./scripts/ss-sens/ss-sens-mqtt-users.sh` - creates pwfile as your user
...
2. Regenerate passwords:
   ```bash
   ./scripts/ss-sens/ss-sens-mqtt-users.sh
   docker compose restart mosquitto
   ```

3. Check ACL file for topic permissions:
   ```bash
   cat config/ss-sens/mosquitto/aclfile
   ```
...
2. Check network connectivity from container:
   ```bash
   docker exec ssv-frigate ping camera-ip
   ```
...
```bash
docker exec ss-sens-cs-redis redis-cli CONFIG SET maxmemory 256mb
```
