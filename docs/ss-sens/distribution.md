...
```
./scripts/ss-sens/ss-sens-release.sh [options]
...
coop-edge-1.3.0-amd64-standard/
  install.sh              # entry point (copy of scripts/ss-sens/ss-sens-install.sh)
  ...
  scripts/
    ...
    coop/
      ss-sens-ctl.sh
      ss-sens-bootstrap.sh
      ss-sens-compose.sh
      ss-sens-credentials.sh
      ss-sens-data-dirs.sh
      coop-env.sh
      ss-sens-mosquitto-tls.sh
      ss-sens-mqtt-users.sh
  images/
...
5. Creates `/opt/ss-sens/` directory tree
...
13. Runs `ss-sens-bootstrap.sh` for first start (generates TLS certs, MQTT users, starts containers)
...
sudo ./install.sh [options]
  ...
  --install-dir DIR      Where to install (default: /opt/ss-sens)
...
```bash
sudo cat /opt/ss-sens/.data/.env
# or
sudo /opt/ss-sens/scripts/ss-sens-credentials.sh --list
```
...
sudo cp your-server.crt /opt/ss-sens/data/coop/mosquitto/certs/server.crt
sudo cp your-server.key /opt/ss-sens/data/coop/mosquitto/certs/server.key
sudo ss-sens-ctl restart mosquitto
```
...
Edit `/opt/ss-sens/data/coop/frigate/config.yml`. Each section has a camera entry that is disabled by default:
...
Apply: `ss-sens-ctl restart frigate`

### Grafana (metrics bundle only)

Access at `http://<edge-ip>:3000`. Default: `admin` / password from `GRAFANA_ADMIN_PASSWORD` in `.env`.

Prometheus datasource is auto-provisioned. Import dashboards from `grafana.com` or from `/opt/ss-sens/config/ss-sens/grafana/provisioning/`.

---

## Resource Tuning

Hardware resource limits are set in `/opt/ss-sens/.data/.env` as `COOP_*` variables. All limits are Docker Compose `deploy.resources.limits` -- containers share unused capacity, these are ceilings not reservations.

```bash
# Edit limits
sudo nano /opt/ss-sens/.data/.env
...
After installation, use `ss-sens-ctl` (available system-wide via `/usr/local/bin/ss-sens-ctl`).

```bash
ss-sens-ctl start             # start all configured services
ss-sens-ctl stop              # graceful stop
ss-sens-ctl restart           # stop then start
ss-sens-ctl status            # container status + live resource snapshot
ss-sens-ctl logs              # stream all logs
ss-sens-ctl logs frigate      # stream logs for one service
ss-sens-ctl ps                # list containers
ss-sens-ctl shell chirpstack  # interactive shell inside a container
ss-sens-ctl update            # pull latest images, recreate changed containers
ss-sens-ctl config            # show resolved docker-compose configuration
ss-sens-ctl env               # show .env (secrets masked)
```
...
```bash
sudo sed -i 's/^COOP_COMPOSE_PROFILES=.*/COOP_COMPOSE_PROFILES=lorawan/' /opt/ss-sens/.data/.env
ss-sens-ctl restart
```
...
```bash
sudo sed -i 's/^COOP_COMPOSE_PROFILES=lorawan,video/COOP_COMPOSE_PROFILES=lorawan,video,metrics/' /opt/ss-sens/.data/.env
ss-sens-ctl restart
```
...
```bash
ls -la /dev/dri/
ss-sens-ctl shell frigate -- vainfo
```
...
```bash
openssl x509 -in /opt/ss-sens/data/coop/mosquitto/certs/server.crt -noout -subject -dates
```

Regenerate if needed:

```bash
HOST=192.168.1.10 /opt/ss-sens/scripts/ss-sens-mosquitto-tls.sh "$HOST"
ss-sens-ctl restart mosquitto
```

**ChirpStack gateway shows "Never seen"**

- Confirm the gateway EUI is registered in ChirpStack (Gateways list)
- Confirm `chirpstack-gateway-bridge` is running: `ss-sens-ctl ps`
