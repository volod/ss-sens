...
Frigate camera streams ──────────────────────────────────┤
                           ...
                           ┌──────────────────────────────┤
                           ▼                              ▼
                   SiteStateAggregator           SensorEventIngestor
                   SensorMeshFusion                       │
...
**Additionally:** `CameraStreamService` registers each Frigate camera in MediaMTX and
...
`CameraStreamService` (started in the FastAPI lifespan) calls the Frigate HTTP API to
discover enabled cameras, then for each camera:

1. Registers an RTSP re-stream path in MediaMTX as `ssv/{camera}`.
2. Starts an `RtspCaptioner` session that writes scene captions to `scene_timeline`.

Camera discovery repeats every 60 seconds (configurable via `CameraStreamService(refresh_sec=…)`),
so cameras added or disabled in Frigate are picked up without a restart.

The MediaMTX path `ssv/{camera}` can be viewed by any RTSP client:
...
`SensorEventIngestor` converts coop observations to `SensorEvent` / `ThreatEvent`
...
docker compose \
  ...
  -f docker/ss-sens/docker-compose.ss-sens.yml \
  up -d
```
...
- [analytics.md](analytics.md) — log analytics CLI (`ss-sens-analytics`)
- [architecture.md](architecture.md) — coop component diagram
