...
- Run `./scripts/ss-sens/ssv-test-usb-cameras.sh` to verify detection

### Step 1: Test USB Camera

```bash
./scripts/ss-sens/ssv-test-usb-cameras.sh
```
...
1. **Add MQTT user** (if new sensor needs its own user):
   ...
   - Add ACL rules in `config/ss-sens/mosquitto/aclfile` for allowed topics
...
4. **Example ACL** (add to `config/ss-sens/mosquitto/aclfile`):
   ```
   user sensor_client
   topic write sensors/#
   topic read sensors/#
...
```bash
# Add RTSP camera
./scripts/ssv/ssv-camera.sh --name front_door --rtsp rtsp://user:pass@ip:554/stream1 --restart
...
| `config/ss-sens/mosquitto/aclfile` | MQTT ACL |
| `data/coop/mosquitto/pwfile` | MQTT passwords |
| `config/ss-sens/chirpstack/` | ChirpStack config |
| `.env` | Passwords, ports |
