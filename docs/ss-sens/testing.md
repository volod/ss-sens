...
docker exec ss-sens-mosquitto mosquitto_sub \
  -h 127.0.0.1 -p 1883 \
...
docker exec ss-sens-cs-postgres pg_isready -U chirpstack

# Redis
docker exec ss-sens-cs-redis redis-cli ping
```
    ...
    steps:
      ...
      - name: Start stack
        run: |
          ...
          APP_ENV=test ./scripts/ss-sens/ss-sens-bootstrap.sh
