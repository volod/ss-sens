...
### Mosquitto (`config/ss-sens/mosquitto/`)

Static configuration (version-controlled): `mosquitto.conf`, `aclfile`.
Generated runtime secrets (written to `data/coop/mosquitto/` by `ss-sens-bootstrap.sh`): `pwfile`, `certs/`.
...
### ChirpStack (`config/ss-sens/chirpstack/`)
...
Template (version-controlled): `config/ss-sens/frigate/config.yml`, `config/ss-sens/frigate/go2rtc_homekit.yml`.
...
`ss-sens-bootstrap.sh` copies the templates to `data/coop/frigate/` on first run.
...
Generated by `scripts/ss-sens/ss-sens-mosquitto-tls.sh`:
...
For production, replace files in `data/coop/mosquitto/certs/` (created by `ss-sens-bootstrap.sh`):
