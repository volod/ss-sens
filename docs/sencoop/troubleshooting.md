...
4. Ensure data directories exist (bind mounts):
   ```bash
   ./scripts/sencoop/sencoop-data-dirs.sh
   ```
...
5. If containers fail with "Operation not permitted" or "Unable to open" on data dirs:
   ...
   - For a fresh start: `./scripts/sencoop/sencoop-clean-data.sh`, then `./scripts/sencoop/sencoop-bootstrap.sh`

6. If you cannot remove `./data/*` (permission denied):
   - `./scripts/sencoop/sencoop-clean-data.sh` fixes ownership and removes data
...
3. Ensure TLS and pwfile were created by your user (not sudo):
   - `./scripts/sencoop/sencoop-mosquitto-tls.sh` - run as normal user
   - `./scripts/sencoop/sencoop-mqtt-users.sh` - creates pwfile as your user
...
3. Check ACL file for topic permissions:
   ```bash
   cat config/sencoop/mosquitto/aclfile
   ```
