...
- Run `./scripts/sencoop/sencoop-test-usb-cameras.sh` to verify detection

### Step 1: Test USB Camera

```bash
./scripts/sencoop/sencoop-test-usb-cameras.sh
```
...
1. **Add MQTT user** (if new sensor needs its own user):
   ...
   - Add ACL rules in `config/sencoop/mosquitto/aclfile` for allowed topics
...
4. **Example ACL** (add to `config/sencoop/mosquitto/aclfile`):
   ```
   user sensor_client
   topic write sensors/#
   topic read sensors/#
...
```bash
# Add RTSP camera
./scripts/sencoop/sencoop-camera.sh --name front_door --rtsp rtsp://user:pass@ip:554/stream1 --restart
...
| `config/sencoop/mosquitto/aclfile` | MQTT ACL |
| `data/coop/mosquitto/pwfile` | MQTT passwords |
| `config/sencoop/chirpstack/` | ChirpStack config |
| `.env` | Passwords, ports |
