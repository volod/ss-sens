...
```bash
./scripts/sencoop/sencoop-release.sh --version 1.3.0 --arch arm64 --bundle standard --yes
```
...
coop-edge-1.3.0-amd64-standard/
  install.sh              # entry point (copy of scripts/sencoop/sencoop-install.sh)
...
Prometheus datasource is auto-provisioned. Import dashboards from `grafana.com` or from `/opt/coop/config/sencoop/grafana/provisioning/`.
...
For more detail see `docs/sencoop/troubleshooting.md`.
