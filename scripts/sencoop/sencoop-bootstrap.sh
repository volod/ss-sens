...
If no arguments are passed, this runs: `./scripts/sencoop/sencoop-compose.sh up -d`
...
if [[ ! -f "$(project_env_file)" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-env.sh" "$ENV"
fi

# Ensure bind-mount directories exist
"$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-data-dirs.sh"
...
if [[ ! -f "$_FRIGATE_LIVE" ]]; then
  ...
  cp "$PROJECT_ROOT_DIR/config/sencoop/frigate/config.yml" "$_FRIGATE_LIVE"
  # Also copy go2rtc homekit config if present
  _GO2RTC="$PROJECT_ROOT_DIR/config/sencoop/frigate/go2rtc_homekit.yml"
...
if [[ ! -f "$_DATA_DIR/coop/mosquitto/certs/server.key" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-mosquitto-tls.sh" "$HOST"
fi
...
if [[ ! -f "$_DATA_DIR/coop/mosquitto/pwfile" ]]; then
  ...
  "$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-mqtt-users.sh"
fi

# Start the stack
if [[ $# -eq 0 ]]; then
  exec "$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-compose.sh" up -d
else
  exec "$PROJECT_ROOT_DIR/scripts/sencoop/sencoop-compose.sh" "$@"
fi
