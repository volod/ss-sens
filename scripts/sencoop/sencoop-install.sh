...
#   sudo ./scripts/sencoop/sencoop-install.sh [options]  # from repo root
#
# Options:
...
if [[ -f "$_SELF_DIR/release.manifest" ]]; then
  ...
  # Running as scripts/sencoop/sencoop-install.sh inside bundle or from the project repo
...
mkdir -p "$INSTALL_DIR"/{docker/sencoop,config,env,logs}
mkdir -p "$DATA_DIR"

cp "$BUNDLE_DIR/docker/sencoop/docker-compose.sencoop.yml" "$INSTALL_DIR/docker/sencoop/"
...
# System-wide command
ln -sf "$INSTALL_DIR/scripts/sencoop/sencoop-ctl.sh" /usr/local/bin/sencoop-ctl
log "Installed /usr/local/bin/sencoop-ctl -> $INSTALL_DIR/scripts/sencoop/sencoop-ctl.sh"
...
# ── Compose profiles ──────────────────────────────────────────────────────────
# Map bundle + metrics flag -> COOP_COMPOSE_PROFILES (read by sencoop-ctl on every start)
case "$BUNDLE" in
...
Environment=SENCOOP_INSTALL_DIR=${INSTALL_DIR}
ExecStart=${INSTALL_DIR}/scripts/sencoop/sencoop-ctl.sh start
ExecStop=${INSTALL_DIR}/scripts/sencoop/sencoop-ctl.sh stop
ExecReload=${INSTALL_DIR}/scripts/sencoop/sencoop-ctl.sh restart
TimeoutStartSec=180
TimeoutStopSec=60
Restart=on-failure
RestartSec=15
...
SENCOOP_INSTALL_DIR="$INSTALL_DIR" \
  "$INSTALL_DIR/scripts/sencoop/sencoop-bootstrap.sh" up -d
...
log ""
log "  Control:"
log "    sencoop-ctl start | stop | restart | status | logs | ps|sencoop-ctl start | stop | restart | status | logs | ps|sencoop-ctl start | stop | restart | status | logs | ps|sencoop-ctl start | stop | restart | status | logs | ps|sencoop-ctl start | stop | restart | status | logs | ps|sencoop-ctl start | stop | restart | status | logs | ps"
log "    systemctlsencoop-ctl start | stop | restart | status | logs | pscoop-stack"
log ""
