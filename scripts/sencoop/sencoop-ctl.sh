...
# Works from the project repo (dev) and as /usr/local/bin/sencoop-ctl (symlink).
#
# Usage:
#   sencoop-ctl <command> [args]
#
# Commands:
...
#
# Environment:
#   SENCOOP_INSTALL_DIR   Override the install directory (set by systemd unit)
...
# SENCOOP_INSTALL_DIR override (useful for multi-install or systemd ExecStart env)
if [[ -n "${SENCOOP_INSTALL_DIR:-}" ]]; then
  _SCRIPTS_DIR="${SENCOOP_INSTALL_DIR}/scripts/sencoop"
fi
...
# ── Helpers ───────────────────────────────────────────────────────────────────
log()  { printf '[sencoop-ctl] %s\n' "$*"; }
die()  { printf '[sencoop-ctl] ERROR: %s\n' "$*" >&2; exit 1; }

_require_env() {
  [[ -f "$(project_env_file)" ]] \
    || die ".env not found at $(project_env_file). Run sencoop-bootstrap.sh first."
}
...
case "$CMD" in
  ...
  start)
    ...
    _compose up -d "$@"
    log "Stack running. Use 'sencoop-ctl status' or 'sencoop-ctl logs'."
    ;;
  ...
  shell)
    [[ -n "${1:-}" ]] || die "Usage: sencoop-ctl shell <service>"
    _apply_compose_profiles
  ...
  *)
    die "Unknown command: $CMD. Run 'sencoop-ctl help' for usage."
    ;;
