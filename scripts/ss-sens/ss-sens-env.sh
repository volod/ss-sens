#!/usr/bin/env bash
# Generate ss-sens stack `.data/.env` from the env templates.
#
# Usage:
#   ./scripts/ss-sens/ss-sens-env.sh [prod|dev|test] [--force]
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage: ./scripts/ss-sens/ss-sens-env.sh [prod|dev|test] [--force]

Generates `.data/.env` for the ss-sens stack.
EOF
  exit 0
fi
...
TEMPLATE="$PROJECT_ROOT_DIR/src/ss_sens/env/${ENV}.env"
...
sed -i "s|COOP_MQTT_PASSWORD=REPLACE_ME|COOP_MQTT_PASSWORD=$(generate_password 16)|"                               "$OUTPUT"
...
project_log "Print credentials with: ./scripts/ss-sens/ss-sens-credentials.sh --list"
