...
#   sudo ./scripts/ss-sens/ss-sens-install.sh [options]  # from repo root
#
# Options:
#   --install-dir DIR      Installation root (default: /opt/ss-sens)
...
if [[ -f "$_SELF_DIR/release.manifest" ]]; then
  ...
  # Running as scripts/ss-sens/ss-sens-install.sh inside bundle or from the project repo
...
mkdir -p "$INSTALL_DIR"/{docker/ss-sens,config,env,logs}
mkdir -p "$DATA_DIR"

cp "$BUNDLE_DIR/docker/ss-sens/docker-compose.ss-sens.yml" "$INSTALL_DIR/docker/ss-sens/"
...
# System-wide command
ln -sf "$INSTALL_DIR/scripts/ss-sens/ss-sens-ctl.sh" /usr/local/bin/ss-sens-ctl
log "Installed /usr/local/bin/ss-sens-ctl -> $INSTALL_DIR/scripts/ss-sens/ss-sens-ctl.sh"

# ── Kernel parameters ─────────────────────────────────────────────────────────
step "Kernel parameters"

cat > /etc/sysctl.d/99-ss-sens.conf << 'SYSCTL'
...
SYSCTL
sysctl -p /etc/sysctl.d/99-ss-sens.conf
...
cat > /etc/logrotate.d/ss-sens << LOGROTATE
${DATA_DIR}/mosquitto/log/*.log {
    ...
    notifempty
    postrotate
        docker exec ss-sens-mosquitto mosquitto_pub -h localhost -t '\$SYS/broker/reload' -n 2>/dev/null || true
    endscript
}
...
LOGROTATE
log "Logrotate: /etc/logrotate.d/ss-sens"
...
cat > /etc/systemd/system/ss-sens-stack.service << UNIT
[Unit]
Description=ss-sens IoT Edge Stack
...
Environment=SS_SENS_INSTALL_DIR=${INSTALL_DIR}
ExecStart=${INSTALL_DIR}/scripts/ss-sens/ss-sens-ctl.sh start
ExecStop=${INSTALL_DIR}/scripts/ss-sens/ss-sens-ctl.sh stop
ExecReload=${INSTALL_DIR}/scripts/ss-sens/ss-sens-ctl.sh restart
TimeoutStartSec=180
TimeoutStopSec=60
Restart=on-failure
RestartSec=15
...
SyslogIdentifier=ss-sens-stack
...
systemctl daemon-reload
systemctl enable ss-sens-stack.service
log "Enabled ss-sens-stack.service (auto-start on boot)"
...
SS_SENS_INSTALL_DIR="$INSTALL_DIR" \
  "$INSTALL_DIR/scripts/ss-sens/ss-sens-bootstrap.sh" up -d
...
log ""
log "  Control:"
log "    ss-sens-ctl start | stop | restart | status | logs | ps"
log "    systemctl start|stop|restart|status ss-sens-stack"
log ""
